...
async function analyzeWithGemini({ images, photoNote }) {
  const prompt = [
    ...
    'Если данных мало, всё равно дай лучший ориентир по видимому фото, но объясни неопределённость в portionNote. Не расширяй диапазон без причины.',
    'Округляй caloriesMin и caloriesMax до ближайших 10 ккал.',
    'caloriesMin и caloriesMax должны описывать только то, что видно на фото и указано в комментарии пользователя.',
